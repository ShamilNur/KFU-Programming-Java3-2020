...
public class AvailabilityServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        AvailabilityForm availabilityForm = objectMapper.readValue(req.getReader(), AvailabilityForm.class);

        List<Room> availableRooms;
        try {
            availableRooms = roomsService.getAvailableRooms(availabilityForm);
        } catch (NoRoomsAvailableException e) {
            availableRooms = roomsService.getAllRooms();
            req.setAttribute("noAvailableRooms", "Нет доступных номеров с этими параметрами. Выберите самостоятельно среди всех.");
        }
